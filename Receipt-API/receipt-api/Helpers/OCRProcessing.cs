...
namespace receipt_api
{
    ...
    public class OCRProcessing
    {
        ...
        public async Task<Receipt> GetReceiptAsync(string uri, string apiKey)
        {
            OCRVisionResponse ocrResponse = await GetOperationResult(uri, Secrets.ApiKey);
        ...
        public async Task<object> CallComputerVisionOCRAsync(byte[] image)
        {
        ...
        public async Task<OCRVisionResponse> GetOperationResult(string apiUri, string apiKey)
        {
        ...
        Receipt ExtractReceiptLines(OCRVisionResponse ocr)
        {
            ...
            foreach (Line line in ocr.recognitionResult.lines)
            {
                string strline = "";
                ...
                int[] bounds = line.boundingBox;
                newReceiptLine.x = bounds[0];
                newReceiptLine.y = bounds[1];
                newReceiptLine.width = bounds[2];
                newReceiptLine.height = bounds[3];
